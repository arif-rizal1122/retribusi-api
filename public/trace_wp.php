...
try {
    ...
    if (count($taxpayers) > 0) {
        ...
        $stmt2 = $pdoModern->prepare("SELECT id, nop, name, address, retribution_type_id FROM tax_objects WHERE taxpayer_id = ?");
        $stmt2->execute([$taxpayerId]);
        $output['modern']['tax_objects'] = $stmt2->fetchAll(PDO::FETCH_ASSOC);
    }
